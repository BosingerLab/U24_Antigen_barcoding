...
while(my $ln = <IN>)
{
	...
	$db{"MB\_$new_id"} = "$id\_MB\_$new_id\t$ln";
...
while(my $ln = <IN>)
{
	...
	if($arr[56] && $arr[56]=~/[A-Z]/ && $arr[167] && $arr[167]=~/[A-Z]/)
	{
		$clonotype{$hcdr3_len}{"Neut\_$new_id"}{'HV'} = $arr[12];
		$clonotype{$hcdr3_len}{"Neut\_$new_id"}{'HJ'} = $arr[14];
		$clonotype{$hcdr3_len}{"Neut\_$new_id"}{'HCDR3'} = $arr[56];
		$clonotype{$hcdr3_len}{"Neut\_$new_id"}{'LV'} = $arr[123];
		$clonotype{$hcdr3_len}{"Neut\_$new_id"}{'LJ'} = $arr[125];
		$clonotype{$hcdr3_len}{"Neut\_$new_id"}{'LCDR3'} = $arr[167];
					...
					{		
						...
						if($lv_check)
						{
							...
							if($lj_check)
							{
								...
								if($lcdr3_check)
								{	
									 print "$s1 $s2 $clonotype{$len}{$s1}{'HV'} $clonotype{$len}{$s1}{'HCDR3'} Found match\n";	
									++$v{$_} for $s1,$s2;
			    					# $uf100->union($s1,$s2);
			    					$uf->union($s1, $s2);
